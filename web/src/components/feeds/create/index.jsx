...
const CreateFeed = props => {
  ...
  const createFeed = async e => {
    ...
    const response = await feeds.create(data, true);
    if (response.status === 200) {
      snackBarContext.openSnackBar('Your feed successfully uploaded, after approving of moderator you will be sent an email, thank you:)');
      onClose();
    } else {
...
const mapStateToProps = state => ({
  formData: getFormValues('createFeed')(state),
